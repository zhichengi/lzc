...
数字单位：节点分类为准确率 %，动态链接预测为 MRR。更新于 2026-09-16。
...
| 43 | ScaDyG（TNNLS 2026） | MOOC 链接预测，seeds 0–4 | 0.931 ± 0.009 | **0.922 ± 0.014**（MRR 选模）；0.915 ± 0.009（AP 选模） | 严格 item 排名协议 **0.204 ± 0.004** | 是 | 是 | 否 | 报告完成；消融已完成（`topo` 塌成随机）；BitcoinAlpha 已完成 | `repro/SCADYG_REPORT.md` |
| 43 | ScaDyG | BitcoinAlpha 链接预测，seeds 0–4 | —（本次不与论文直接对照） | **0.719470 ± 0.006932**（official MRR） | — | 是 | — | — | 第二数据集 L1 闭环；SNAP 原始数据 + 723000 秒切片 = 226 快照 | `repro/SCADYG_REPRO_LOG.md` |
...
| 10 | Stable-ChebNet（NeurIPS 2025） | Peptides-func，LRGB（AP ↑） | 70.32 ± 0.26 | **67.869**（1 run，末 epoch） | — | 是 | 部分 | — | 官方原样已跑；差 −2.45，待步骤 7（末 epoch 评测 / 损失口径） | `repro/STABLE_CHEBNET_REPRO_LOG.md` |
