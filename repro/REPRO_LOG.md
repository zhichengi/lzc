...
> **已冻结**（R-GCTD-5，2026-09-13）：R-GCTD-1…5 全部收口，issue 决定不发出。
...
- [x] 向作者发 issue：**决定不发出**（2026-09-13），草稿保留在 `repro/gctd_issue_draft.md`
- [x] 冻结条目（R-GCTD-5，2026-09-13）：见文末「冻结」节

---

**本条线已冻结。** 若重启，触发条件是作者给出 Table 2 完整超参。
...
## 2026-09-13 收口决定：不发出 issue

R-GCTD-4 的 issue 草稿（`repro/gctd_issue_draft.md`）**决定不发出**。原因：向作者索取
未公开的 Table 2 超参属于低优先级的对外沟通，当前不需要对外动作即可把本条线收口；
草稿连同 GCond 对照证据一并保留，日后若需要可直接发出。目标仓库已确认为
`nicolasrsantos/gctd`（`has_issues: true`，维护者即论文一作）。

---

## 2026-09-13 冻结（R-GCTD-5）

**范围**：Table 2 的三个小图格（Cora 1.3% / Citeseer 0.9% / Pubmed 0.08%），本地建议
命令 `--lr_rec 0.01 --edge_topk 12` + 塌缩重试 + 超点配额，10 seed（Citeseer 另加
`lr_rec=0.001` 对照 10 seed）。不涉及大图（Flickr / Reddit / Ogbn-arxiv）。

### 三级结论

| 级别 | 判定 | 依据 |
|------|------|------|
| L1 管线闭环 | **是** | 源码、数据、压缩、合成图评测、原图 val 选模 / test 评测全链路可重复；退出码全 0；评分脚本可从原始日志重算并与 CSV 交叉校验 |
| L2 数值量级 | **部分** | Pubmed 77.90 vs 79.9±0.2（差 2.0，σ 7 倍）接近；Cora 65.96 vs 81.4±1.6（差 15.4）、Citeseer 64.92 vs 76.8±0.4（差 11.9）未达到 |
| L3 统计一致 | **否** | 除 Pubmed 外 σ 均比论文大一个数量级（5.9×–19.3×），不是同分布的重跑 |

### 关键证据

1. **差距不是数据集特有**：三个数据集全部低于论文，幅度 −15.4 / −11.9 / −2.0。
2. **不是"未走塌缩降 lr 路径"**：Citeseer 主动用 `lr_rec=0.001` 只从 64.92 抬到
   66.45（+1.53），σ 仍是论文 13 倍；且 1e-3 下核**不塌缩**（与 Pubmed 的机制不同）。
3. **不是评测链路**：GCond 官方压缩图（论文 Table 2 原始产物）走同一 harness 得到
   Cora 79.34±0.69 / Citeseer 69.64±0.59，均命中论文 GCond 列（79.8±1.3 / 70.5±1.2）。
   同样 35 个超点，GCond 79.3、我们 65.96。

### 剩余假设（未验证，也不再继续）

- 官方 Table 2 所用的完整超参未写入 README（其采用 wandb 贝叶斯搜索）；
  `lr_rec` / `R` / `add_ratio` / `lr_gnn` / `weighted` / `drop_ratio` 的实际取值未知。
- 官方默认 `lr_rec=0.001` + 0.05 阈值在 Cora 上必得完全图（1225/1225 边），本身即
  与论文"ReLU / 阈值带来稀疏"的描述不一致。
- K-Means 初始化与簇内特征取法（论文写簇内平均，代码用 class+split 原型）。

### 冻结规则

**不再在 Cora/Citeseer/Pubmed 上扫超参**，不再追加 seed。若要重启本条线，触发条件是
作者给出 Table 2 完整配置（issue 未发出，草稿保留）。官方克隆提交 `785cfc9`，
复现补丁 `repro/gctd-repro.patch`。
